    ...
    if (!existsSync(options.fontsDir)) {
        throw new CliFailure(
            `Fonts directory not found: ${options.fontsDir}\n` +
                ...
                "in packages/charts to regenerate the .fonts-cache, or pass --fonts <dir> pointing at " +
                "licensed TTF copies.",
        )
    }
    ...
    if (fontFiles.length === 0) {
        throw new CliFailure(
            `No .ttf files found in ${options.fontsDir}. Run \`bun run extract-font-metrics\` in ` +
                "packages/charts, or pass --fonts <dir> containing TTF copies of the brand fonts.",
        )
    }
