...
Use the `charts` binary from `@buildcanada/charts` to validate chart inputs and render deterministic SVG/PNG chart outputs. Prefer this CLI path when the task is to produce chart files rather than embed a React component.
