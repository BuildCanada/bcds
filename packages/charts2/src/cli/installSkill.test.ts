...
import { CHARTS2_SKILL_NAME, installSkill, resolveInstallTargets, resolveSkillSourceDir } from "./installSkill.ts"
...
function makeSourceSkill(): string {
    const dir = join(makeTmpDir(), CHARTS2_SKILL_NAME)
    mkdirSync(dir, { recursive: true })
    writeFileSync(
        join(dir, "SKILL.md"),
        [
            "---",
            `name: ${CHARTS2_SKILL_NAME}`,
...
describe("resolveSkillSourceDir", () => {
    it("finds the bundled charts2 CLI skill", () => {
        const sourceDir = resolveSkillSourceDir()
        expect(readFileSync(join(sourceDir, "SKILL.md"), "utf8")).toContain(`name: ${CHARTS2_SKILL_NAME}`)
    })
})
        ...
        expect(resolveInstallTargets({ agent: "claude", path: root })).toEqual([
            {
                agent: "custom",
                skillsRoot: root,
                destination: join(root, CHARTS2_SKILL_NAME),
            },
        ])
    })
...
describe("installSkill", () => {
    it("copies the skill to every target", () => {
        ...
        const destination = join(skillsRoot, CHARTS2_SKILL_NAME)
    ...
    it("refuses to overwrite unless --force is set", () => {
        ...
        expect(readFileSync(join(destination, "SKILL.md"), "utf8")).toContain(`name: ${CHARTS2_SKILL_NAME}`)
    })
})
