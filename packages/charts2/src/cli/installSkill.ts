/**
 * charts install-skill - install the bundled charts2 CLI agent skill.
 *
 * The skill is stored in packages/charts2/skills during development and copied
...
export const CHARTS2_SKILL_NAME = "charts2-cli"
...
export function resolveSkillSourceDir(): string {
    ...
    const candidates = [
        // Published build: dist/cli/installSkill.js -> dist/skills/charts2-cli
        resolve(cliDir, "../skills", CHARTS2_SKILL_NAME),
        // Source and monorepo dev: src/cli/installSkill.ts -> skills/charts2-cli
        resolve(cliDir, "../../skills", CHARTS2_SKILL_NAME),
    ]
    for (const candidate of candidates) {
        if (existsSync(join(candidate, "SKILL.md"))) return candidate
    }
    throw new CliUsageError(`Could not find bundled skill "${CHARTS2_SKILL_NAME}"`)
}
...
export function resolveInstallTargets(args: { agent?: string; path?: string }): SkillInstallTarget[] {
    if (args.path !== undefined) {
        ...
        return [{ agent: "custom", skillsRoot, destination: join(skillsRoot, CHARTS2_SKILL_NAME) }]
    }
    ...
    const makeTarget = (name: "codex" | "claude"): SkillInstallTarget => {
        ...
        return { agent: name, skillsRoot, destination: join(skillsRoot, CHARTS2_SKILL_NAME) }
    }
...
export function installSkill(args: { sourceDir: string; targets: SkillInstallTarget[]; force: boolean }): void {
    for (const target of args.targets) {
        if (basename(target.destination) !== CHARTS2_SKILL_NAME) {
            throw new CliUsageError(`Internal error: destination must end with ${CHARTS2_SKILL_NAME}`)
        }
        ...
        process.stdout.write(`installed ${CHARTS2_SKILL_NAME} for ${target.agent}: ${target.destination}\n`)
    }
}
...
export const installSkillCommand = defineCommand({
    meta: {
        name: "install-skill",
        description: "Install the bundled charts2 CLI agent skill into Codex, Claude, or a custom skills directory",
    },
    args: {
        agent: {
            type: "string",
        ...
        path: {
            type: "string",
            description: "Custom agent skills directory; installs <path>/charts2-cli and ignores --agent",
        },
        force: {
            type: "boolean",
            description: "Replace an existing charts2-cli skill at the destination",
            default: false,
        },
    },
    run({ args }) {
