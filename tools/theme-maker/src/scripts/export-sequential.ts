...
async function main() {
  const files = await readdir(join(cwd(), "src/out/charts/sequential"));
  console.log({ files });
  ...
  fb.addLine(`export const sequential = {`);
  results.forEach((item) => {
    fb.addLine(`Chart${item},`, 1);
  });
  fb.addLine(`};`);
  const out = fb.build();
  await Bun.write(join(cwd(), "src/out/charts/sequential/index.ts"), out);
}
